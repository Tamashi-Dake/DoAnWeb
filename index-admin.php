...
?>
    ...
    if(!isset($_POST['search'])){
      ...
      $search = $_POST['search']; 
      $sql_search = "select * from webbaiguixe.vehicleinout inner join  webbaiguixe.parkitem on vehicleinout.cardID=parkitem.cardID where vehicleinout.display=1 and ( vehicleinout.cardID='".$search."' or vehicleinout.licensePlate='".$search."') and type=1 order by vehicleinout.time desc limit 1";
    ...
    ?>
      ...
      ?>
          <p>   
          ID thẻ: <?php echo($row['cardID']) ?>
          </p>
          <p>
          Biển số: <?php echo($row['licensePlate']) ?>
          </p>
          Khu: <?php echo($row['areaName']) ?>
          </p>
          <p>
          Ô đỗ: <?php echo($row['location']) ?>
          </p>
<?php
          if($row['type']==0){
            $_SESSION['rp_error'] = "ko có xe trong bãi";
            //echo $_SESSION['rp_error'];
           echo "<p>". $_SESSION['rp_error'] ."</p>";
         }
          }
      }
      else{
